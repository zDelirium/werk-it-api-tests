...
public class TestAerobics {
    ...
    static void setupAll() {
        baseURL = "https://staging.tiered-planet.net/werk-it-back-end/aerobics";
    }
    ...
    public void testExistingAerobicsGet() {
        ...
        String sutURL = String.format("%s/user/%d", baseURL, expectedValuesMap.get("[0].userId"));

        expectedValuesMap.forEach((field, expectedValue) -> httpRequestFunctions.assertGetEndpointFieldValue(field,
    ...
    public void testValidAerobicsPost() {
        int userId = 1;
        String sutURL = String.format("%s/user/%d", baseURL, userId);
